...
/* Esquema de color (idéntico a “Service on CSR”, solo color de fondo) */
const BG_COLOR = {
...
export default function Section4() {
  ...
  const [transferOnToggle, setTransferOnToggle] = useState(false);
  ...
  /* ----- Ticket final padding ----- */
  const padTicket = () => {
    const digits = res.ticketFinal.replace(/\D/g, "");
    if (!digits) return;
    set("ticketFinal", digits.padStart(15, "0").slice(-15));
  };
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <label className="flex flex-col gap-0.5 text-[10px] font-semibold uppercase">
              <span>
                RESOLVED{" "}
                {missOutcome && <span className="text-red-600">*</span>}
              </span>
              {/* wrapper que cambia de fondo */}
              <div
                className={`rounded border ${
                  res.outcome ? `${BG_COLOR[res.outcome]} border-transparent` : "border-gray-300"
                }`}
              >
                <select
                  value={res.outcome}
            ...
            <label className="flex flex-col gap-0.5 text-[10px] font-semibold uppercase">
              <span className="flex items-center gap-1">
                ...
                <button
                  type="button"
                  onClick={() => setTransferOnToggle(!transferOnToggle)}
          ...
          {(isTech || isFollowUp) && (
            <div
              className={`mt-2 grid ${
                isTech ? "grid-cols-4" : "grid-cols-2"
              } gap-2`}
            >
              {/* CBR2 (solo Tech) */}
              {isTech && (
              ...
              <label className="flex flex-col text-[10px] font-semibold uppercase">
                <span>
                  ...
                  {(isTech ? missTech : missFUp) && !res.techDate && (
                    <span className="text-red-600">*</span>
                  )}
                </span>
                <input
              ...
              <label className="flex flex-col text-[10px] font-semibold uppercase">
                <span>
                  ...
                  {(isTech ? missTech : missFUp) && !res.techTime && (
                    <span className="text-red-600">*</span>
                  )}
                </span>
                <select
              ...
              {isTech && (
                <label className="flex flex-col text-[10px] font-semibold uppercase">
                  ...
                  <input
                    readOnly
                    value="Yes | $200. Cx aware and agree"
          ...
          <div
            className={`mt-2 grid ${
              isTicket ? "grid-cols-2" : "grid-cols-1"
            } gap-2`}
          >
            {/* Ticket final */}
            <label className="flex flex-col gap-0.5 text-[10px] font-semibold uppercase">
              <span>
                TICKET {missFinal && <span className="text-red-600">*</span>}
              </span>
              <input
                inputMode="numeric"
                maxLength={15}
            ...
            {/* Ticket extra (BOSR / NC / EMT) */}
            {isTicket && (
              <label className="flex flex-col gap-0.5 text-[10px] font-semibold uppercase">
                <span>
                  {ticketLbl} {missTicket && <span className="text-red-600">*</span>}
                </span>
                <input
